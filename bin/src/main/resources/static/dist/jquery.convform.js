   ...
   // console.log(allAnswers);
    ...
    this.parameters.eventList.onInputSubmit(this, function(){
        ...
        if(this.next()){
            ...
            if(index==3 && allAnswers[2]=="Yes")
            filterAnswers.push(allAnswers[3]);
            if(index>3 && index<=7)
            {
                filterAnswers.push(allAnswers[index]);
            }
            if(index==9 && allAnswers[9]=="No")
            {
                filterAnswers=[];
                allAnswers=[];
                index=1;
            ...
            if(index==10)
            {
                filterAnswers.push(allAnswers[10]);
                console.log(filterAnswers);

                

                axios.post('http://localhost:3000/api/chatHistory',{
                    name: `${localStorage.getItem('name')}`,
                    email:`${localStorage.getItem('email')}`,
                    budget: `${filterAnswers[0]}`,
                    class: `${filterAnswers[1]}`,
                    color: `${filterAnswers[2]}`,
                    engine: `${filterAnswers[3]}`,
                    carChecked: `${filterAnswers[4]}`,
                    delivery: `${filterAnswers[5]}`,

                })
                .then(function(response){
                    console.log(response);
                    axios.post('http://localhost:3000/api/updateChat',{
                        name: `${localStorage.getItem('name')}`,
                        email:`${localStorage.getItem('email')}`,
                        budget: `${filterAnswers[0]}`,
                        class: `${filterAnswers[1]}`,
